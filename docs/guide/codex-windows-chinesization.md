...
1. 本教程主要适合只有纯国内网络环境的用户。
2. 如果你已经有可用的网络代理工具，不建议使用此方案。通常打开代理后重启 Codex，界面就会自动显示中文。
3. 汉化效果不保证 100% 成功，尤其是 Windows 10，可能出现“已经汉化但不完全”的情况。
4. 汉化工具只影响本机 Codex 相关文件和启动入口，不会读取或上传你的 API 密钥。
...
安装完成后，`Codex-windows汉化工具` 目录中会多出 `Codex 汉化版.bat` 文件。

以后需要双击 `Codex 汉化版.bat` 打开汉化版 Codex。开始菜单里的 Codex 入口仍然可能打开英文版。
